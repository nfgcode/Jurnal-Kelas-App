...
@section('content')
    @php
        $totalPresensi = array_sum($presensi) ?: 1;
        $sparkJurnal = array_values($pengisian);
    ...
    <x-page-head
        ...
        <span class="select-hifi" style="width: 178px">{{ now()->translatedFormat('F Y') }}</span>
    ...
    <div class="grid-row" style="grid-template-columns: repeat(6, minmax(0, 1fr))">
        ...
        <x-kpi label="Jadwal Aktif" :value="number_format($kpi['jadwal'], 0, ',', '.')"
               :spark="$datar" caption="seluruh rombel" />
        <x-kpi label="Total Jurnal" :value="number_format($kpi['jurnal'], 0, ',', '.')"
               :spark="$sparkJurnal" :delta="$trenJurnal" caption="14 hari terakhir" />
    </div>

    {{-- Analytics row --}}
    <div class="grid-row" style="grid-template-columns: minmax(0, 366fr) minmax(0, 259fr) minmax(0, 227fr) minmax(0, 245fr)">
        <x-card title="Pengisian Jurnal" :meta="'14 hari · rata-rata ' . round(array_sum($pengisian) / max(1, count($pengisian))) . '/hari'">
            <x-barchart :series="$pengisian" />
        </x-card>

        <x-card title="Presensi Siswa" :meta="now()->translatedFormat('F Y')">
        ...
        <x-card title="Kehadiran Guru" :meta="now()->translatedFormat('F Y')">
                ...
                <tbody>
                    @forelse ($jurnalTerbaru as $jurnal)
                        ...
                        <tr>
                            ...
                            <td class="is-muted">{{ $jurnal->guru?->name }}</td>
