...
return new class extends Migration
{
    ...
    {
        // Keep the lowest id per (jurnal_id, siswa_nis). MySQL needs the self-join
        ...
        if (DB::connection()->getDriverName() === 'mysql') {
            DB::statement('DELETE p1 FROM presensi p1 JOIN presensi p2
                ON p1.jurnal_id = p2.jurnal_id AND p1.siswa_nis = p2.siswa_nis AND p1.id > p2.id');
        } else {
            DB::statement('DELETE FROM presensi WHERE id NOT IN (
                SELECT MIN(id) FROM presensi GROUP BY jurnal_id, siswa_nis)');
        }

        Schema::table('presensi', function (Blueprint $table) {
            $table->unique(['jurnal_id', 'siswa_nis'], 'presensi_jurnal_siswa_unique');
        });
    }
