...
class PesanError
{
     ...
     * that broke is the asset pipeline. {@see Ikon} is a plain PHP map, so using
     * it here adds no such dependency, and keeps one list of icons instead of
     * two that could drift apart.
     */
    public static function svg(string $ikon): string
    {
        return Ikon::ada($ikon)
            ? Ikon::svg($ikon)
            : Ikon::svg('exclamation-triangle');
    }
}
